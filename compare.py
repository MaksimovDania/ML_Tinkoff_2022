        ...
        with open(paths[1], "r") as file2:
            ...
            for i in range(len(max_text)):
                try:
                    ...
                    distances.append(len(max_text[i]))  # добавляем в разницу длину строки - столько вставок нужно,
                    # чтобы превратить пустую строку в исходную
    return np.mean(normalize(distances))  # возвращаем среднее среди нормализованных значений, значения с [0, 1]
